...
export default function HotlineNumbers() {
  ...
  const fetchHotlines = async () => {
    ...
    try {
      ...
      console.log('Hotlines API response:', res.data);
      setHotlines(res.data.hotlines || []);
    } catch (e) {
      console.error('Hotlines API error:', e);
      setError(e.response?.data?.detail || e.message || 'Failed to load hotlines');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-900" data-testid="hotlines-page">
      ...
      <main className="px-4 py-6 max-w-2xl mx-auto">
        ...
        <div className="mt-4 space-y-3" data-testid="hotlines-list">
          {loading && (
            <div className="text-white/60 text-sm text-center py-10" data-testid="hotlines-loading">
              Loading hotlines...
            </div>
          )}

          {!loading && hotlines.map((hotline) => (
            <button
